...
const db = await loadDb("reading",`db/reading/reading${rand}.db`);
const data=db.exec(`select * from readinglong ORDER BY RANDOM()LIMIT 1`)[0].values[0]
...
const chinese=data[2]
const questions=JSON.parse(data[3])
const options=JSON.parse(data[4])
const answers=JSON.parse(data[5])
...
watch(yourAnswers,(newVal)=>{
    ...
    const currentScore=isCorrects.value.reduce((a,b)=>{return a+b*(parseInt(point.value)/5)},0)
