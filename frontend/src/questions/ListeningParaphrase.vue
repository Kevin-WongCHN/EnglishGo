    ...
    <el-card style="width: 80%; margin: 20px">
      ...
      <div style="font-size: 25px;">
        <div v-for="(i, index) in options" :key="index">
          <el-radio v-model="answer" :value="index" :disabled="isFinished">{{ i }}</el-radio>
        </div>
      </div>
      ...
      <template #footer>
        <div v-if="isFinished">
          {{ question }}<br/>
          您的答案:{{ answer ? options[parseInt(answer)] : "未作答" }} <br/>
          正确答案:{{ truth }} <br/>
          您答{{ isCorrect ? "对" : "错" }}了！
        </div>
      </template>
